...
int ApolloApp::Spin() {
  /**
   * Zuo commented on 2018-03-29
   * call AdapterManager::Init();
   * 为什么需要用到适配器模式？用来Protocol和ROS通信？
   * 这里的Init()是纯虚函数，被子类override了，比如perception模块，那就是调用Perception::Init()
   */
  auto status = Init(); 
  if (!status.ok()) {
   ...
   * 同上，Start()是纯虚函数，这里调用了对应模块实现的Start()
  */
  status = Start();
  if (!status.ok()) {
