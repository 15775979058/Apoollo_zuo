...
bool DAGStreaming::InitSubnodes(const DAGConfig& dag_config) {
  ...
  AINFO << "subnode_config'size = " << subnode_config.subnodes().size();
  AINFO << "edge_config'size = " << edge_config.edges().size();
  //-- Zuo added on 2018-04-08
  ...
  for (auto& edge_proto : edge_config.edges()) {
    ...
    //-- Zuo added on 2018-04-08
    AINFO << "edge_config'from_node = " << edge_proto.from_node();
    AINFO << "edge_config'to_node = " << edge_proto.to_node();
    //-- Zuo added on 2018-04-08
