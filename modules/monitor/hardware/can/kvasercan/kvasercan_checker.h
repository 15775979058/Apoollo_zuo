...
#ifndef MODULES_MONITOR_HARDWARE_CAN_KVASERCAN_KVASERCAN_CHECKER_H_
#define MODULES_MONITOR_HARDWARE_CAN_KVASERCAN_KVASERCAN_CHECKER_H_
...
class KvaserCanChecker : public HwCheckerInterface {
 public:
  static const char KVASER_CAN_NAME[];

  /// Returns a HW status code from ntstatus.
  static HardwareStatus::Status kvasercan_result_to_hw_status(
      canStatus stat);

  /// Returns a HW status message from ntstatus.
  static std::string kvasercan_result_to_message(canStatus stat);

  explicit KvaserCanChecker();

  virtual ~KvaserCanChecker() {}
 ...
 private:
  ...
  DISALLOW_COPY_AND_ASSIGN(KvaserCanChecker);
};
